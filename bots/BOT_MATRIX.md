...
| `ops_lt` | Deck 🗂️ | Ops | Inbox, Quill, Chronos, Tasker, Purse | `quality` Sonnet Max | kanban (dispatch), AIPass, session_search, memory (ops meta), file `_agent/ops_lt/**`, discord `#fleet` + `#drafts` read | terminal, code_execution, browser, computer_use, delegation, web, mail (read or send), todoist MCP, calendar mutate, OSB write, Monarch | **none** |
