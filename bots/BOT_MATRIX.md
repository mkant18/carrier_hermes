...
| `repair_planner` | Rigger 🪢 | `claude-opus-4-5` anthropic → `claude-sonnet-4-6` anthropic → `grok-4.5` xai-oauth | file `_agent/maintenance/**` (read+write), session_search, memory, skills, AIPass | terminal, code_execution, browser, computer_use, delegation, web, mail, todoist, calendar, kanban | none |
