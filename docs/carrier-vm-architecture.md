...
A bot's container failing never affects other bots. The pattern mirrors the
OpenMausBot design (see `server/drivers/claude.ts` — `containerProxyEnv`,
`brokerSocketPath`).
...
```
  ...
  Token counting rule: only the single completed response per turn is ever
  read for usage — never accumulated from streaming intermediates.
```
...
## OpenMausBot-Compatible Fleet Layer

Alongside the canonical 20-bot production fleet above, `scripts/carrier_vm_manager.py`
also drives a smaller, OpenMausBot-compatible compatibility fleet
(`FLEET_BOT_IDS`: `coding_lt`, `research_lt`, `helm`, `ob1`, `viking`, `peers`)
built on top of `scripts/carrier_bot_vm.py`.

### BotVM / Fallback Mode

`get_bot_vm(bot_id)` (in `carrier_bot_vm.py`) returns a singleton `BotVM` per
`bot_id` within a process, preventing duplicate container names from racing
starts. When Docker is unavailable (e.g. a dev machine without WSL2 Docker),
`BotVM` automatically falls back to a **temp directory workspace** — all exec
commands run as local subprocesses instead of `docker exec`, keeping the code
path identical in both modes:
...
`start_fleet_vms()`, `stop_fleet_vms()`, `vm_status()`, and `exec_all()` in
`carrier_vm_manager.py` orchestrate this compatibility fleet, parallelized
with `ThreadPoolExecutor` so starting several bots takes the time of one
start, not several. Compose definition lives at `docker/carrier-bot-vm.yml`.

```bash
python scripts/carrier_vm_manager.py status --openmaus   # six-bot fleet status
python scripts/carrier_vm_manager.py start                # start the whole compat fleet
...
Passing an explicit `bot_id` to `start`/`stop`/`exec` still targets a single
bot in the canonical 20-bot fleet exactly as before — the compatibility fleet
only activates when no `bot_id` is given.
