...
def send_to_whatsapp(text: str) -> bool:
    """Send a message to the WhatsApp group via the Hermes gateway outbound CLI."""
    if not WA_GROUP:
        print("bridge: CARRIER_WA_GROUP_ID unset — cannot send to WhatsApp yet", file=sys.stderr)
        return False
    # hermes gateway send (platform whatsapp) — chief_of_staff owns the WA session
    cmd = ["hermes", "-p", "chief_of_staff", "gateway", "send",
           "--platform", "whatsapp", "--to", WA_GROUP, "--text", text]
