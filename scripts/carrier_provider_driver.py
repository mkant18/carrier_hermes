...
The driver shape follows OpenMausBot's contracts.ts / drivers/claude.ts SPI
pattern: any provider can be swapped without touching task dispatch logic.

Key rule from upstream:
  "Token counting: ONLY count from the turn_complete event, never accumulate
  intermediates." Every driver here reads usage exclusively from the single
  completed response per turn — never from streaming chunks.

Usage:
    from scripts.carrier_provider_driver import route_request, Message

    resp = route_request([Message(role="user", content="Hello")], context={}, bot_id="coding_lt")
"""
...
# ── OpenMausBot-compatible token accounting ────────────────────────────────────
...
# ── OpenMausBot-compatible SPI contract ─────────────────────────────────────────
# Documents the same driver shape as OpenMausBot's contracts.ts ProviderDriver
# interface. carrier's actual drivers below implement the richer BaseDriver ABC
# (send/is_available) rather than this Protocol (route/on_turn_complete); this
# is kept as the translated reference contract for ports from upstream.

@runtime_checkable
class ProviderDriver(Protocol):
    """SPI contract every OpenMausBot-style provider driver must satisfy."""
...
# ── OpenMausBot-compatible driver registry ──────────────────────────────────────
# Slug-keyed lookup for consumers ported directly from OpenMausBot, which
# addressed drivers as DRIVERS["anthropic"] / DRIVERS["xai"] / DRIVERS["ollama"].
# These are the raw (non-billing-guard-wrapped) driver instances; route_request()
# above remains the billing-enforced entry point for carrier fleet bots.
...
def get_driver(provider_kind: str) -> BaseDriver:
